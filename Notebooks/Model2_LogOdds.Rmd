...
```{r}
whitelogit <- glm(winner_white ~ white_bishop_rem + white_knight_rem + white_pawn_rem +
                 white_queen_rem + white_rook_rem, data = chess_games_pieces_remaining)
confint(whitelogit)
```

```{r}
blacklogit <- glm(winner_black ~ black_bishop_rem + black_knight_rem + black_pawn_rem +
...
```{r}
novice_alllogit = glm(winner_white ~ 
                 1 + pawn_diff + knight_diff + bishop_diff + rook_diff + queen_diff,
...
```{r}
classa_alllogit = glm(winner_white ~ 
                 1 + pawn_diff + knight_diff + bishop_diff + rook_diff + queen_diff,
...
```{r}
ll.null <- alllogit$null.deviance/-2
ll.proposed <- alllogit$deviance/-2

(ll.null - ll.proposed) / ll.null

1 - pchisq(2*(ll.proposed-ll.null), df = (length(alllogit$coefficients) - 1))
```
